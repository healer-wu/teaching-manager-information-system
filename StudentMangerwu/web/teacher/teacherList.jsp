	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'class_id',title:'授课班级',width:500,
					formatter: function(value,row,index){
						if (row.class_id){
							var clazzList = $("#clazzList").combobox("getData");
							for(var i=0;i<clazzList.length;i++ ){
								//console.log(clazzList[i]);
								if(row.class_id == clazzList[i].id)
				...
				// },
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							var data = {class_id:clazzid, name:name,password:password,sex:sex,mobile:phone,qq:qq};
	...
	// , #add_courseList
		...
		// 		$(this).combobox("setValue", data[0].id);
	  	...
	  	$("#add_clazzList").combobox({
			url: "classservlet?method=getClazzList&t="+new Date().getTime()+"&from=stuFrom",
	  		onLoadSuccess: function(){
				//默认选择第一条数据
				var data = $(this).combobox("getData");
				$(this).combobox("setValue", data[0].id);
		...
		// 		$(this).combobox("setValue", data[0].id);
	  	...
	  	$("#editDialog").dialog({
				...
				// },
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							// var data = {id:id, sn:number, name:name,sex:sex,mobile:phone,qq:qq,};
			...
			onBeforeOpen: function(){
				...
				$("#edit_number").textbox('setValue', selectRow.sn);
				...
				var clazzid = selectRow.class_id;
...
<body>
	...
	<div id="toolbar">
		...
		<div style="float: left;margin-top: 4px">&nbsp;&nbsp;姓名：<input id="search_teacher_name" class="easyui-textbox" name="search_teacher_name" /></div>
		<div style="margin-top: 3px">&nbsp;&nbsp;授课班级：<input id="clazzList" class="easyui-textbox" name="clazz" />
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			...
	    			<td colspan="3">
						<input id="add_clazzList" style="width: 200px; height: 30px;" class="easyui-textbox" name="class_id" />
	    			</td>
    	...
    	<form id="editForm" method="post">
	    	...
	    	<table id="editTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			...
	    			<td colspan="3"><input id="edit_number" data-options="readonly: true" class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="sn" data-options="required:true, validType:'repeat', missingMessage:'请输入工号'" /></td>
	    			<td style="width:80px"></td>
	    		</tr>
	    		<tr>
	    			<td>姓名:</td>
				...
				<tr>
					<td>班级:</td>
					<td><input id="edit_clazzList" style="width: 200px; height: 30px;" class="easyui-textbox" name="class_id" /></td>
				</tr>
	    	</table>
	    </form>
	</div>
